...
public class DynamicContentPage extends BasePageObject {
    ...
    public void verify_content_change_after_refresh(int time_to_refresh) {
        for (int refresh_time = 0; refresh_time < time_to_refresh; refresh_time++) {
            ...
            Assert.assertNotEquals(first_content_before_change, first_content_after_change);
            Assert.assertNotEquals(second_content_before_change, second_content_after_change);
            Assert.assertNotEquals(third_content_before_change, third_content_after_change);
        }
    }
